...
export function Navbar() {
  ...
  return (
    <>
      ...
      {/* Mobile — bottom bar */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around pt-3 pb-[max(env(safe-area-inset-bottom),0.75rem)] z-50">
        {links.map((link) => (
          <NavLink
            key={link.path}
            to={link.path}
